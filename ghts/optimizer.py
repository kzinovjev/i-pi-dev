...
def z_increment(ghts, cv_set, sigma, gradsigma_mod, gradq_mod2, bk):
    return c_increment(sigma, gradsigma_mod, gradq_mod2, rp.get_dq_dz(ghts, cv_set),
                       rp.get_grad_grad_dq_dz(ghts, cv_set), bk)


def n_increment(ghts, cv_set, sigma, gradsigma_mod, gradq_mod2, bk):
    return c_increment(sigma, gradsigma_mod, gradq_mod2, rp.get_dq_dn(ghts, cv_set),
                       rp.get_grad_grad_dq_dn(ghts, cv_set), bk)


def d_increment(ghts, cv_set):
    mean_cv_value = rp.mean_beads(getattr, cv_set, 'value')
    return orthogonal(mean_cv_value - ghts['z'], ghts['n'], ghts['Minv'])
...
def c_increment(sigma, gradsigma_mod, gradq_mod2, dq_dc, grad_grad_dq_dc, bk):
    return bk * sigma.value * gradsigma_mod * np.matmul(sigma.gradient, dq_dc) - \
